...
/// バックグラウンドアラーム音の共有プレイヤー。
/// CLLocationModule（バックグラウンド時）と AppDelegate（kill 後再起動時）の両方から使用する。
...
class CLLocationModule: RCTEventEmitter {
    ...
    private func stopMonitoringInternal() async {
        ...
        // monitor が nil（アプリ kill 後再起動時など）でも、OS に永続化された
        // CLMonitor の条件を確実に削除するため、名前で取得してクリアする。
        let m = monitor ?? (await CLMonitor("StationAlarmMonitor"))
    ...
    /// JS 側から呼び出してバックグラウンドアラーム音を停止する。
    /// CLLocationModule 経由・AppDelegate 経由どちらで起動された音も AlarmAudioPlayer.shared で管理するため、
    /// このメソッド1つで確実に停止できる。
    @objc func stopAlarm(
        _ resolve: @escaping RCTPromiseResolveBlock,
        reject _: @escaping RCTPromiseRejectBlock
    ...
    private func handleGeofenceEnter(identifier: String) async {
        ...
        // 現在のアクティブセッションと sessionId が一致しない場合は
        // 前回セッションの残存ジオフェンスによる誤発火なので無視する。
